...
21.1b **步级(S-unit)派发粒度上限 — 小模型短上下文友好**:Phase 内每次 `Agent()` 派发对应一个 S-unit(22.6 表一行);单步 ≤`config.json#subagent.step_max_files`(默认 2)文件、≤`step_max_lines`(默认 100)行、预估 ≤`step_max_minutes`(默认 15)分钟——比 21.1 的 Phase 级上限收紧一档。依据:小模型在短上下文与长上下文下执行质量差异巨大,15 分钟内可完成的单步其上下文增长有限;超限 = 计划无效,回炉再拆而**不是**换更大模型（机器校验已生效：check-plan-dispatch.sh attest 时逐行校验时长 NNmin≤step_max_minutes/输入 token≤step_max_files，超限拒绝；不可解析 SKIPPED 显式化）
