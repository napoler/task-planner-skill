...
- **动态激活边界锚定 + 守卫观察面扩展（task-v090）** — 落地用户确认的选项 1+2：`references/critical-rules.md` 纯追加 **39.7 动态激活边界与禁自建副本**（39.7.1 /workflow 自动激活=ZCode harness 系统内置链路（命令正文强制加载 dynamic-workflows 官方 bundled skill + 四工具拒跑门控），技能侧零补建/禁越层建用户级 command/副本 / 39.7.2 禁自建 dynamic-workflows 副本 P0 锚（用户级目录优先于 bundled=永久遮蔽漂移；发现→报告+Rule 36.4 逐项确认删除）/ 39.7.3 matcher 观察扩展=唯一允许操作面：ZCode 运行位 cli config.json matcher 扩至 `Write|Edit|Agent|CreateWorkflow|AmendWorkflow|SaveWorkflow|EvalWorkflowSnippet`，zcode-pretooluse.sh 39.7.3 观察分支仅注入一行 [workflow-observe] 提醒（39.4 并行豁免登记指引），exit 0 恒放行零阻断；register-hooks-cj.ts（Claude 运行位）matcher 同步扩围但 command 保持 check-scope.sh（workflow 四工具无 file_path→空路径 exit 0 天然放行）——39.5 机器校验边界原文维持，观察面扩展≠守卫面扩展；39.5 尾追加观察面注记。`scripts/selftest-workflow-orchestration.sh` 追加 WF-13..16（39.7 三子条锚 / 用户级两 skills 目录无 dynamic-workflows 负断言 / pretooluse 观察分支在位且分支内无 exit 2 / register-hooks matcher 锚），Total 12→16。零新 config 键（39.6 范式延续）。harness 侧 `~/.zcode/cli/config.json` PreToolUse matcher 扩围为运行态变更（P0 基础设施，用户 09-25 显式授权）。

- **动态工作流编排接入（task-v088）** — 落地用户指令「task-planner 接入 /workflow（动态工作流编排）」：`references/critical-rules.md` 末尾纯追加 **Rule 39 动态工作流编排**（39.1 触发纪律=用户显式点名 /workflow 才路由 dynamic-workflows（CreateWorkflow 编排），未点名保持 21.4 串行派发零改动，复刻官方 "explicit request is binding" / 39.2 skill 加载前置门槛（Create/Amend/Save/EvalSnippet 未加载 dynamic-workflows 时拒绝，saved: 唯一豁免）/ 39.3 四机制映射表（失败→AmendWorkflow cache / 断点→ResumeWorkflowRun / 人工升级→ResolveWorkflowQuestion(dwfq) / 模板沉淀→SaveWorkflow；每 Phase 验收→phase()+report()+world.run）/ 39.4 21.4 串行铁律在 workflow run 内部豁免（显式调用期登记制，非改写 21.4 原文；用户要求串行传 max_concurrency=1）/ 39.5 机器校验边界如实披露（官方文档未提及 check-dispatch 覆盖 workflow 内部，机器校验归 harness 侧）/ 39.6 机制=零新 config 键，与 v087 同范式）。SKILL 5 处联动（协同路由矩阵 dynamic-workflows 行 + Rule 39 摘要行 + C27 检查项 + 「Rules 1-38」→「1-39」索引级联 2 处 + 协同路由行内注记，555→558 行净增 3 行）；4 索引文档（SKILL.md/CLAUDE.md/README_zh.md/skills/task-planner/README.md）「Rules 1-38」→「1-39」级联 6 处；4 既有 selftest 行数上限锚 555→558（batch-pilot/execution-stability/knowledge-brief/skill-collab）。守护：`scripts/selftest-workflow-orchestration.sh`（新，WF-01..12 静态断言：Rule 39 条款锚 + SKILL 联动 + 索引级联双向钉 + config 键数 40 零新增）。本仓全量 27 selftest 预期 0 FAIL。部署：三位实体位 IDENTICAL + push 远端。

- **新任务边界判定（task-v087）** — 落地用户指令「自动分析是否属于新任务；如是则开新 task_plan.md，避免不相干内容混在一起」：`references/critical-rules.md` Rule 8 族**纯追加 8.1 新任务边界判定（D 类）**（判定=新指令对照当前 task_plan.md 三要素 `## Goal` + `scope_files`/执行范围 + 交付物，均无主题/范围/交付物关联→D 类新任务；处置=开新计划目录 `plans/{new-task-id}/`（init-session 全流程）+ 旧计划原样保留（不标 superseded，区别于 C 类矛盾）+ notepad 一行判定登记；判定顺序 **D 先于 A/B/C**（有关联回退 A 类；无活跃计划时 D 类 N/A）；Rule 8 原文零改动，纯追加避 v082 插入级联教训）；`SKILL.md` 四处纯增量（用户新指令表加 D 行 + 判定顺序行 / 特判段「新增任务边界判定（Rule 8.1 — task-v087）」/ C12 检查项扩为 D/A/B/C / 高频漂移触发时机行兼容保留，551→555 行）；`scripts/zcode-userpromptsubmit.sh` [plan-note] 文案加 D 类判定指引（Rule 8.1 锚 + 不相干内容禁止混入提示，fail-open 语义不变）；`references/todo-sync.md` S5 hook 说明加 D 类分支（开新计划目录 + 旧计划 Todo 映射原样保留）。守护：`scripts/selftest-task-boundary.sh`（新，TB-01..11 静态锚：8.1 在位+语义锚 / Rule 8 原文未改 / SKILL D 行+特判段+C12 / hook note+职责注释 / todo-sync D 分支）+ 4 个既有 selftest 行数断言 552→555 对齐。本仓全量 26 selftest **441 PASS / 0 FAIL**（基线 430+11 新断言）。零新 config 键（判定为 LLM 行为面，机器只守护条款在位，Rule 35 同范式）。

- **难度分级轻量档+项目多模板（task-v086）** — 落地用户指令「执行太慢，是否统一模板导致；可按难度提供不同级别模板；一个项目支持多个模板每次只加载 1 个」：`references/critical-rules.md` 末尾纯追加 **Rule 38 任务难度分级与轻量档**（38.1 判定三条件机器可测=计划 frontmatter `plan_tier: mini` ∧ scope ≤2 文件 ∧ ≤15min ∧ 单模块+MISMATCH 提示 / 38.2 mini·standard·full 三档矩阵（mini=新建 `templates/variant/mini-lite-type.md` 49 行 2 Phase VC≥2 跳 FMEA/知识储备/委派统计/Batch/Chain/Drift 六仪式区块；standard=现有 13 variant；full=general）/ 38.3 mini-lite 区块白名单 5 项 / 38.4 门控豁免锚表 5 点（attest FMEA RC=2 SKIP + check-complete VC 5→2·V-N 等效阈值 0·委派 floor 0.7→0.0 + check-plan-dispatch S-unit 豁免+MISMATCH 三档 + knowledge-brief 流程层降档；非 mini 路径零影响铁律，master 三方对照逐字节实证）/ 38.5 机制键 `plan_tier_enforce`）；`config.json` 新增 `plan_tier_enforce`（enforce/warn/off 默认 warn，env `TASK_PLANNER_PLAN_TIER_ENFORCE` 覆盖）；`scripts/init-session.sh` tier 分流（第 3 参/env `TASK_PLAN_TIER=mini`，variant 定制优先提示）+ **S6 项目多模板极简支持**（`--list` 列全部可用模板不建文件 / 项目 plan-templates 目录 `*-type.md` 自动并入合法类型 / `default` 指针文件 > env `TASK_TEMPLATE_DEFAULT` > general 回落 / 自造模板复制后头部插 frontmatter 标识——每次任务只加载 1 个模板，零注册表零新目录）；`scripts/check-template-type.sh` 提取链补**第三形态** `<!-- template_type: X -->` 注释 form（CR 首轮 BLOCKER：写入侧注释形态与读取侧提取链契约未对齐，mini 主路径 enforce 档拒锁，S7 修复+PT-28 钉住双端闭环）；14 既有模板 +1 行 `plan_tier: standard` 纯注释标记。SKILL 联动 1-37→1-38 索引级联（3 处）+C26+摘要行（549→551 行）。守护：`scripts/selftest-plan-tier.sh`（新，PT-01~28）；9 个既有 selftest 锚修复（4 处 SKILL 行数断言 549→552 + 宽容锚 1-3[5-7]→1-3[5-8] 等）。本仓全量 25 selftest **430 PASS / 0 FAIL**（基线 402+28 新断言）。CR 两轮（首轮 CHANGES_REQUESTED 4 发现 → S7 修复 → 二轮 APPROVED）。deferred：D1 项目自造模板名不入 34.1 白名单（enforce 档拒锁，既有设计边界，留独立任务）；D2 簿记措辞更正。
