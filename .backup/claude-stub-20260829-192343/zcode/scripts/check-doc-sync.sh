...
# [2026-08-28] 修复：原实现读取 config#sync_interval_calls（该键不存在）且读出的 VAL 从未使用
# （死代码）；改为 jq 读取 plan_update_interval_minutes，配置路径依次尝试
# OPENCODE_SKILL_ROOT → ~/.zcode → ~/.claude，均缺失时兜底 10 分钟。
CONFIG_FILE=""
for _c in "${OPENCODE_SKILL_ROOT:-}/config.json" \
