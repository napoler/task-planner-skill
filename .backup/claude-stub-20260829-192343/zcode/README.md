...
├── config.json                   # 阈值配置
├── templates/
│   ├── task_plan.md              # 主计划（含 Drift Log 区）
│   ├── findings.md               # 外部记忆（需求/调研/决策）
│   ├── verification.md           # VC 明细 + Phase Gates + Goal Gate
│   ├── progress.md               # 会话日志（动作/测试/错误）
│   └── notepad-learnings.md      # 经验记录（New Requests/What Worked）
├── references/
